...
public class PowerOfLargeNumbers {
	...
	private static BigInteger ithDigit(BigInteger n, int i) {
		/**
		 * n = number whose digits we need to extract b = radix in which the
		 * number if represented i = the ith bit (ie, index of the bit that
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < T; i++) {
			BigInteger val = powerIterativeEff(in.nextBigInteger(),
					in.nextBigInteger());
